Project Overview
This project explores how upstream bioprocess parameters influence biologics production yield. The goal is to build a model for optimizing mixing time, temperature, pH, and dissolved oxygen (DO) to improve yield. Due to proprietary / confidential challenges of obtaining and using real data from the biotech industry, real manufacturing data cannot be used. As a result, a synthetic dataset was generated to simulate representative upstream manufacturing conditions in fermentation and cell culture.
...
Data Source
Due to the proprietary nature of the biologics manufacturing industry, real process data was not available for this project. Instead, a synthetic dataset was generated to reflect realistic upstream conditions in fermentation and cell culture. The dataset simulated batch-level production records, structured in a table format, where each row represents a batch. 

Data Structure
 - Input features: mixing time, temperature, pH, dissolved oxygen
 - Target variable: final batch yield

The synthetic data was generated considering domain knowledge of biologics operations to ensure realistic parameter ranges and biologically plausible relationship.

Methods
Methods includes the following components:
